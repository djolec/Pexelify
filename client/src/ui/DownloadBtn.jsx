  ...
  return (
    <div ref={downloadMenu} className="relative flex flex-row gap-3">
      ...
      <button
        ...
        className="flex flex-row items-center gap-2 rounded-full bg-[var(--primary)] text-[var(--on-primary)] sm:px-4 2xl:px-6"
      >
        <span className="hidden text-lg font-semibold sm:block 2xl:text-4xl">
          Download
        </span>
        <Download className="h-10 w-auto px-2 py-2 sm:px-0 sm:py-2 2xl:h-[70px] 2xl:p-3" />
      </button>
      ...
      {downloadOpen && (
        <motion.ul
          ...
          className={`absolute left-0 top-[calc(100%+4px)] z-20 flex w-36 origin-top translate-y-1 flex-col gap-0 overflow-hidden rounded-lg border-[1px] border-[var(--outline)] bg-[var(--background)] text-[var(--on-background)] sm:w-full`}
        >
          {data.map(([key, value], index) => {
            return (
              ...
              >
                <button
                  className="h-full w-full px-3 py-1 text-left text-base 2xl:py-3 2xl:text-3xl"
                  onClick={() => {
                    downloadMedia(
                      value,
                      `${
